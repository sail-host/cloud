...
import (
	...
	"github.com/sail-host/cloud/internal/log"
...
func setOutput(logger *logrus.Logger, config config.LogConfig) {
...
func (s *MineFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	detailInfo := ""
	if entry.Caller != nil {
		function := strings.ReplaceAll(entry.Caller.Function, "github.com/sail-host/cloud/internal/", "")
